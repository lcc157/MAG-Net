...
class vgg16(Network):
    ...
    def build_rpn(self, net, is_training, initializer):
        ...
        # Create RPN Layer
        rpn = slim.conv2d(net, 512, [3, 3], trainable=is_training, weights_initializer=initializer, scope="rpn1_conv/3x3")
        # rpn = slim.conv2d(rpn1, 256, [1, 1], trainable=is_training, weights_initializer=initializer, scope="rpn_conv/3x3")

        self._act_summaries.append(rpn)
        rpn_cls_score1 = slim.conv2d(rpn, self._num_anchors * 2, [1, 1], trainable=is_training, weights_initializer=initializer, padding='VALID', activation_fn=None, scope='rpn_cls_score1')

        attention_map = slim.conv2d(rpn_cls_score1, 1, [1, 1], trainable=is_training, weights_initializer=initializer, scope="attention_map")
        rpn_cls_score = tf.multiply(rpn_cls_score1, attention_map)
